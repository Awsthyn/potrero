...
function DetalleVoluntario(props){
    if(props.voluntarios.length)
     var {firstName,lastName,createdAt,state,email,id,isActive,linkedin,cv,phone,birthday}=props.voluntarios.filter(v=>v.id == props.id)[0]

    const classes = useStyles()
        console.log(props.voluntarios,props.id)
        ...
        <div className= {classes.root}>
                    ...
                    {props.voluntarios.length ?
                           ...
                           <Grid container spacing={4}>
                                 <Grid item xs={12}>
                                        ...
                                        <Grid container justify="center" spacing={2}>
                                                <Typography className={classes.typo}  component="h2" gutterBottom>
                        ...
                        <Grid item xs={12}>
                                    ...
                                    <Grid container justify="center" spacing={2}>
                                            <Typography className={classes.typo}  component="h2" gutterBottom>
                                            <b>Fecha de Postulación:</b> {moment(createdAt).format('LLL')}hs
                                            </Typography>
                                    </Grid>
                                    <Grid container justify="center" spacing={2}>
                                            <Typography className={classes.typo}  component="h2" gutterBottom>
                                    ...
                                    <Grid className={classes.typo} container justify="center" spacing={2}>
                                         <b>Curriculum:</b>{!cv ? 'Sin CV':'' }
                                         <br />
                                        {cv ? <object data={`http://localhost:3001/uploads/${cv}`} 
                                    ...
                                    <Grid className={classes.typo} container justify="center" spacing={2}>
                                                            <button key={`aceptar${id}`} className={`${state ==='pendiente'? "btn-warning":"btn-success "} btn border`}  onClick={() => handleStatusChange({id,firstName,lastName})}>
                                                                 <i className={`${state === 'pendiente'? "fa fa-toggle-off":"fa fa-toggle-on"}`}></i> 
                                                                 {state ==='pendiente'? 'Aceptar':'Activo'}                                                                
                                                            </button>
                                                            <button key={`rechazar${id}`} name={Number(id)} className="btn btn-danger border" onClick={e => handleDeletion(id)}>
                                                                 <i name={id} className="fa fa-trash">Rechazar</i>                                                                   
                                                            </button>
